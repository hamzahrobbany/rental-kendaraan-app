// src/components/dashboard/orders/EditOrderForm.tsx
'use client';

import { useState, useEffect, useCallback, FormEvent } from 'react';
import { User, Vehicle, Order, OrderStatus, PaymentMethod, Role } from '@prisma/client';
...
export default function EditOrderForm({ order, onSuccess, onCancel }: EditOrderFormProps) {
  ...
  const [form, setForm] = useState({
    ...
    remainingAmount: order.remainingAmount?.toString() || '', // BARU: Inisialisasi remainingAmount
  ...
  // Reset form state jika order prop berubah
  useEffect(() => {
    setForm({
      userId: order.userId.toString(),
      vehicleId: order.vehicleId.toString(),
      ...
      endDate: new Date(order.endDate).toISOString().split('T')[0],
      depositAmount: order.depositAmount?.toString() || '',
      remainingAmount: order.remainingAmount?.toString() || '', // Reset remainingAmount
      paymentMethod: (order.paymentMethod || '') as PaymentMethod | '',
      orderStatus: order.orderStatus,
      adminNotes: order.adminNotes || '',
      pickupLocation: order.pickupLocation || '',
    ...
    setCurrentStep(1); // Reset ke langkah pertama saat order baru dipilih
  }, [order]);
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        ...
        setUsers(data.filter(u => u.role === Role.CUSTOMER));
      } catch (err: any) {
  ...
  // Fetch Vehicles berdasarkan tanggal yang dipilih (dan mengecualikan pesanan saat ini)
  const fetchVehiclesByDate = useCallback(async () => {
    ...
    try {
      ...
      if (form.startDate && form.endDate) {
        url += `?startDate=${form.startDate}&endDate=${form.endDate}&excludeOrderId=${currentOrderId}`;
      } else {
        url += `?excludeOrderId=${currentOrderId}`;
      }
      ...
      const currentOrderVehicleInFilteredList = vehiclesData.find(v => v.id === order.vehicleId);
      if (!currentOrderVehicleInFilteredList && order.vehicle) {
          const originalVehicle = {
              id: order.vehicle.id,
              name: order.vehicle.name,
              licensePlate: order.vehicle.licensePlate,
              dailyRate: order.vehicle.dailyRate,
          } as Vehicle;
          if (!finalVehicles.some(v => v.id === originalVehicle.id)) {
              finalVehicles.unshift(originalVehicle);
          }
      }
      
      setAvailableVehicles(finalVehicles);

      if (finalVehicles.length > 0) {
        if (!finalVehicles.some(v => v.id.toString() === form.vehicleId)) {
          setForm(prev => ({ ...prev, vehicleId: finalVehicles[0].id.toString() }));
        }
        setSelectedVehicleDailyRate(parseFloat(finalVehicles.find(v => v.id.toString() === form.vehicleId)?.dailyRate.toString() || '0'));
      } else {
  ...
  }, [form.startDate, form.endDate, form.vehicleId, order.id, order.vehicle]);
  ...
  useEffect(() => {
    ...
    if (!isNaN(total) && !isNaN(deposit)) {
      ...
      setForm(prev => ({ ...prev, remainingAmount: calculatedTotalPrice }));
    }
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      toast.error('Gagal Memperbarui Pesanan', {
        description: err.message || 'Terjadi kesalahan saat memperbarui pesanan.',
      });
    } finally {
      setLoading(false);
    }
  ...
  if (availableVehicles.length === 0 && form.startDate && form.endDate && !dataLoading) {
    const isCurrentVehicleOnlyOption = availableVehicles.length === 1 && availableVehicles[0].id === order.vehicleId;
    if (!isCurrentVehicleOnlyOption) {
  ...
  // Pesan awal jika tidak ada kendaraan sama sekali di sistem (dan bukan mode edit)
  if (availableVehicles.length === 0 && !form.startDate && !form.endDate && !dataLoading) {
